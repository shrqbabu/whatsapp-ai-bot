...
export class BusinessHoursRepository {
  static async listBySessionId(sessionId: string): Promise<BusinessHourEntity[]> {
    const db = getDatabase();
    const rows = await db.query<any>(
      'SELECT * FROM business_hours WHERE session_id = $1 ORDER BY day_of_week ASC',
      [sessionId]
    );
  ...
  static async updateSchedule(
    ...
    for (const item of schedules) {
      const existing = await db.queryOne<any>(
        'SELECT id FROM business_hours WHERE session_id = $1 AND day_of_week = $2',
        [sessionId, item.day_of_week]
      );
  ...
  private static mapEntity(row: any): BusinessHourEntity {
    return {
      ...row,
