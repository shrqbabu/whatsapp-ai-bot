...
import { ConversationEntity, ConversationsRepository } from '../database/repositories/conversationsRepository.js';
import { MessageEntity, MessagesRepository } from '../database/repositories/messagesRepository.js';
...
import { AIHistoryMessage } from './providers/AIProvider.js';
  ...
  static async processAutoReply(params: {
    ...
    if (fixedMessageText) {
      ...
      try {
        ...
        const conversationHistory: AIHistoryMessage[] = rawHistory
        ...
        logError(
          { userId, sessionId, conversationId: conversation.id, event: 'AI_GENERATION_FAILED' },
          error,
          'Failed generating AI reply'
        );
        return null;
      }
    }
    ...
    logEvent(
      { userId, sessionId, conversationId: conversation.id, event: 'AI_REPLY_SCHEDULED' },
      `Scheduling reply in ${delaySeconds}s: "${replyText.slice(0, 50)}..."`
    );
    ...
    return ReplyScheduler.schedule(delaySeconds, async () => {
      ...
      if (isTakeover) {
        logEvent(
          { userId, sessionId, conversationId: conversation.id, event: 'AI_REPLY_ABORTED' },
          'Manual takeover was activated while reply delay was running'
        );
        return null;
      }
      ...
      logEvent(
        { userId, sessionId, conversationId: conversation.id, waMessageId, event: 'MESSAGE_SENT' },
        `AI Auto-Reply sent to ${chatJid}`
      );
