...
export class MessageDebouncer {
  ...
  private static activeJobs: Map<string, DebouncedMessageJob> = new Map();
  ...
  static enqueue(params: {
    ...
    if (existingJob) {
      ...
      logEvent(
        { sessionId, conversationId, event: 'DEBOUNCE_APPENDED' },
        `Debounce: Appended message (${existingJob.messages.length} messages buffered)`
      );
      ...
      logEvent(
        { sessionId, conversationId, event: 'DEBOUNCE_STARTED' },
        `Debounce: Started timer for ${debounceSeconds}s`
      );
    }
  }
...
export class ReplyScheduler {
   ...
   */
  static async schedule(delaySeconds: number): Promise<void> {
